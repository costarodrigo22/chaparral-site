		...
		<StepperContext.Provider value={{ previousStep, nextStep }}>
			<div className='flex'>
					...
					{steps.map((step, index) => (
						<>
							...
							{index !== steps.length - 1 && (
								<div
									className={cn(
										'w-[2px] h-[40px] my-2 bg-black rounded-full ml-[12px]',
										currentStep !== index && index === 0 && 'opacity-50'
									)}
								></div>
							)}
				...
				<div className='w-full flex flex-col pl-7'>
					<strong className='text-base mb-2'>Criar conta</strong>

					<span>
						Olá! Você está a poucos passo de saborear o melhor açaí do mundo.
					</span>
					<span className='flex'>
						<p className='text-[#FBA301] mr-[3px]'>Conclua seu cadastro</p> e
						embarque nessa experiência deliciosa!
					</span>
