...
export default function DeliveryOrPickupSelector() {
	...
	async function handleSelectDelivery(value: string) {
		if (value === 'Entrega') {
			...
			try {
				...
				toast.error(
					`Algo deu errado: ${JSON.stringify(errorRequest.response?.data)}`
				);
			}
	...
	async function handleSelectLocalPickUpChange(value: string) {
		...
		try {
			...
			const verifyAddressOmie = await axios.post(
				`${process.env.NEXT_PUBLIC_BASE_URL}/api/without/omie/filter_client`,
				{ cnpj_cpf: unformatCPF(userLogged.document) }
			);

			if (!verifyAddressOmie.data.endereco) {
				console.log('elemento sem endereço');

				route.push('');
				// console.log('endereço do cliente: ', verifyAddressOmie.data.endereco);
			}
							...
							<div>
								{listAddress?.length === 0 && (
									<span
										...
										className='text-[#16A6FF] text-base cursor-pointer hover:underline transition-all'
									>
										Necessário cadastrar um endereço para prosseguir
									</span>
								)}

								{selection === 'Retirada' && listAddress?.length > 0 && (
									<div className='w-full py-4'>
