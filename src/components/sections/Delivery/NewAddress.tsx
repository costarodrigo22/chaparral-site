...
import { formatCEP, getJustNumbers, unformatCPF } from '@/lib/utils';
...
export default function NewAddress({ open, onClose }: IModalUpdateAddress) {
  ...
  const handleSubmit = form.handleSubmit(async (formData) => {
    ...
    try {
      const addressCreated = await mutateAsync({
        cep: getJustNumbers(addressStep.cep),
  ...
  return (
    <Dialog open={open} onOpenChange={handleCloseModal}>
      <DialogContent className="max-h-[90vh] max-w-[95vw] 2xl:max-w-[1000px] overflow-y-auto overflow-x-auto">
        ...
        <form onSubmit={handleSubmit}>
              ...
              <Input
                {...form.register('addressStep.cep', {
                  onChange: (event) => {
                    ...
                    form.setValue('addressStep.cep', formatCEP(cep));

                    if (getJustNumbers(cep).length === 8) {
                      handleGetAdressByCep(getJustNumbers(cep));
                    }
                  },
                })}
                id="cep"
          ...
          <div className="flex gap-5 w-full mt-4 flex-col sm:flex-row">
            <div className="w-full">
